...
image: "/images/writing/ai-inbound-architecture.svg"
...
The more AI agents move into real enterprise work, the less the problem looks like a prompt problem.

It starts looking like an inbound architecture problem.
...
That has been the practical lesson from recent work across Easentic and OpsRabbit.
...
## What Easentic Made Concrete

Easentic has been a good reminder that inbound architecture is not abstract theory. It shows up in small product details.

Over the last few weeks, the work touched Slack onboarding, Slack DMs, channel ingestion, Jira scope filters, Teams marketplace onboarding, Teams Home, Teams personal chat, follow-up routing, setup cards, install callbacks, project selection, report delivery, and issue links.
...
- Slack onboarding needed live Jira validation because the workspace was still being connected.
- Later Slack DM project selection needed existing integrated project records because the workspace was already configured.
- Slack channel ingestion needed an internal listener fanout so messages could become durable evidence instead of one-off chat events.
- Jira scope filters had to apply consistently to follow-up reads, ingestion paths, and create paths so shared boards did not leak context.
- Teams onboarding had to reach parity with Slack while respecting Teams-specific concepts: Home tabs, setup cards, channel restore behavior, personal chat routing, Graph channel listing, and invoke responses.
- Teams and Slack both needed project selection, follow-up routing, confirmation checkpoints, and delivery context, but they could not be implemented as copy-paste variants of the same UI.
...
- 7Targets operational knowledge inside the workspace that includes WhatsApp Business, Twilio, and webhook-based communication paths
...
That is what recent Easentic and OpsRabbit work made clear to me.
